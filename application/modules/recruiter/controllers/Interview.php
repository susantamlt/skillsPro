...
class Interview extends MX_Controller 
{
   ...
   public function interview_list() {
		...
		foreach($result['aaData'] as $row){
			...
			$row[5] = '<a href="'.base_url('recruiter/interview/interview_view/').$row[0].'" title="View Record" data-toggle="tooltip"><i class="glyphicon glyphicon-eye-open" ></i></a>&nbsp;&nbsp;<a href="'.base_url('recruiter/interview/interview_form/').$row[0].'" title="Edit Record" data-toggle="tooltip"><i class="glyphicon glyphicon-edit" ></i></a>&nbsp';
	...
	/*public function interview_form($id=" ") {
		...
		$data['suser'] = $this->interview_model->get_prospect_title($data);
		$this->load->view('common/recruiter-top');
		$this->load->view('recruiter/interview/interview_form',$data);
		$this->load->view('common/recruiter-bottom');
		}*/
	public function interview_form($id="") {
		$this->load->model('interview_model');
