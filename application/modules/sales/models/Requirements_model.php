<?php class Requirements_model extends CI_Model
{
	...
	public function get_requirements_list($date='') {
		...
		$tableName5 = 'requirement_source';
		$tableName6 = 'users';
		$columns   = array(
			"$tableName.requirement_id",
			"$tableName.requirement_title",
		...
		$orderby     = "";
		$joinMe      = "left join $tableName1 on $tableName1.requirement_id=$tableName.requirement_id left join $tableName2 on $tableName2.client_id=$tableName.client_id left join $tableName3 on $tableName3.org_id=$tableName.org_id left join $tableName5 on $tableName5.requirement_id=$tableName.requirement_id left join $tableName6 on $tableName6.user_id=$tableName5.source_id";
	...
	public function requirements_save($date='') {
		$rdate = array('org_id'=>$date['org_id'],'user_id'=>$date['user_id'],'no_requirement_fullfilled'=>$date['no_requirement_fullfilled'],'expected_date_of_closure'=>date('Y-m-d',strtotime($date['expected_date_of_closure'])),'requirement_status'=>$date['requirement_status']);
	...
	public function source_save($date='') {
		...
		if($id==0){
			$this->db->select('ps_id');
			$this->db->from('requirement_source');
			$this->db->where('requirement_id',$date['requirement_id']);
			$this->db->where('org_id',$date['org_id']);
			//$this->db->where('source_id',$date['source_id']);
			$datas = $this->db->get()->result_array();
			if(empty($datas) || ($date['ps_id']==$datas[0]['ps_id'])){
				if(isset($date['ps_id']) && $date['ps_id']!=''){
					$_id = $date['ps_id'];
					unset($date['ps_id']);
					$this->db->where('ps_id',$_id);
					$this->db->update('requirement_source',$date);
					$id=5;
				} else {
