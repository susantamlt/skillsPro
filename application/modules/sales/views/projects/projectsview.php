				...
				<div class="row">
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
						<div class="card">
							...
							<div class="body">
								<fieldset>
									<legend><?php echo $urlname; ?> Opening Information <span style="float: right;font-size: 15px;"><input type="checkbox" name="sendMsg" id="sendMsg" style="position: relative;opacity: 1;left: 0;" />&nbsp;Send Contract&nbsp;&nbsp;<input type="checkbox" name="recMsg" id="recMsg" style="position: relative;opacity: 1;left: 0;" />&nbsp;Receive Contract</span></legend>
								...
								<fieldset>
									...
									<div class="form-group" style="clear:both;">
										<label class="col-md-12"> <?php echo $urlname; ?> Description: </label>
										<div class="col-md-12"><?php echo $jobs[0]['prospect_description']; ?></div>
									</div>
									<div class="form-group" style="clear:both;">
										<label class="col-md-12"> Technical Requirements: </label>
								...
								<div class="form-group" style="clear:both;">
									<div class="col-md-12">
										...
										<a href="<?php echo site_url('sales/projects/') ?>" class="btn btn-default">Cancel</a>
									</div>
								</div>
							</div>
						</div>
			...
			$(document).on('click','#sendMsg',function(){
				if ($(this).is(':checked')) {
					if(!confirm('Are you sure you want send contract to client?')){
						...
						$.ajax({
							url: "<?php echo site_url('sales/projects/sendcontract') ?>",
							type: 'POST',
			...
			$(document).on('click','#recMsg',function(){
				if ($(this).is(':checked')) {
					if(!confirm('Are you sure you got confirmation from client?')){
						...
						$.ajax({
							url: "<?php echo site_url('sales/projects/receivecontract') ?>",
							type: 'POST',
							data: {'id':$('#prospect_id').val(),'user_id':$('#user_id').val(),'org_id':$('#org_id').val()},
							success: function(res) {
								var resD = $.parseJSON(res);
