<script type="text/javascript">
	...
	$("#btn_results").click(function(){
		var exam_id=$(this).data('val');
		window.location="<?php echo WEBDIR.ROOT;?>students/list_results.php?exam_id="+exam_id;
	});
			...
			$.ajax({
				...
				success:function(data){
					if(data.result==1){
						...
						setTimeout(function(){window.location="my-exams.php";},3000);
					}
				}
			});
			return 1;
	...
	function save_question_answers() {
		...
		}else if(options_type_to_check == 'TXT'){
		...
		if ((option_chosen != null && option_chosen != '') || allVals != '' || option_chosen_txt != '') {
			$.ajax({
				...
				data:{'question_id':$("#question_id").val(),'options_type_to_check':options_type_to_check,"option_chosen":option_chosen,"allVals":allVals,"option_chosen_txt":option_chosen_txt},
				dataType:"json",
				success:function(data){
					if(data.result==1){
						var $input = $('#question_n');
